...
class BertConnectionLayer(nn.Module):
    ...
    def forward(self, input_tensor1, attention_mask1, input_tensor2, attention_mask2, co_attention_mask=None, use_co_attention_mask=False):
        ...
        attention_output1, attention_output2 = self.biOutput(bi_output2, input_tensor1, bi_output1, input_tensor2) # 说明bi_output2才是vision分支的cross-attention输出
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [copy.deepcopy(layer) for _ in range(config.num_hidden_layers)] # 12 layers
        )
        self.v_layer = nn.ModuleList(
            [copy.deepcopy(v_layer) for _ in range(config.v_num_hidden_layers)] # 6 layers
        )
    ...
    def forward(
        ...
        for v_layer_id, t_layer_id in zip(self.v_biattention_id, self.t_biattention_id):
            ...
            assert self.fixed_t_layer <= t_end # fixed住多少层不训练
            ...
            for idx in range(t_start, t_end): # encoder L1次文本输入
                txt_embedding, txt_attention_probs = self.layer[idx](txt_embedding, txt_attention_mask)
                if output_all_attention_masks:
                    all_attention_mask_t.append(txt_attention_probs)
...
class BertTextPooler(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # to the first token.
        first_token_tensor = hidden_states[:, 0]    # used token
...
class VILBertForVLTasks(BertPreTrainedModel):
    ...
    def forward(
        ...
        vision_logit = self.vision_logit(self.dropout(sequence_output_v)) + ((1.0 - image_attention_mask)* -10000.0).unsqueeze(2).to(dtype=torch.float32)
